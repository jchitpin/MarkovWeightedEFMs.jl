function trie_adj_at_root(#
    ...
    function traverse_trie(#
        prefix::Vector{Int64},
        t::Dict{Vector{Int16}, @NamedTuple{id::Int64, children::Vector{Int16}}},
        #t::Dict{#
            #Vector{Int16},
    ...
    traverse_trie(Int64.([I]), t, T, A, E, show_all)
    return A, E, t
end
...
function label_nodes(#
    t::Dict{#
        Vector{Int16},
        @NamedTuple{id::Int64, children::Vector{Int16}}
    },
)
    labels = [collect(keys(t))[i][end] for i in 1:length(t)]
    idx = [collect(values(t))[i].id for i in 1:length(t)]
...
# Edge attributes (reaction fluxes)
...
# Edge attributes (reaction indices)
function label_edge_fluxes(R::Matrix{<:Real})
    edge_attr = Vector{Int16}()
    cis = findall(>(0), R)
    cis = getindex.(cis, [1 2])
    sorted_cis = cis[sortperm(cis[:,1]),:]
    for e in eachrow(sorted_cis)
        push!(edge_attr, R[e[1],e[2]])
    end
    return edge_attr
end
...
function tree_layout(#
    ...
    function get_coords(#
        ...
        nid = t[prefix].id
        pid = length(prefix) == 1 ? 0 : t[prefix[1:(end-1)]].id
    ...
    for r in 1:length(uniqs)
        ...
        if length(all_coords) == 1
            p[:node_pos][][all_coords[].nid] = GeometryBasics.Point{2, Float32}(#
                mean([xl₀, xr₀]),
                y₀
            )
        else
            ...
            p[:node_pos][][all_coords[1].nid] = GeometryBasics.Point{2, Float32}(#
                xl₀,
                y₀
            )
            p[:node_pos][][all_coords[end].nid] = GeometryBasics.Point{2, Float32}(#
                xr₀,
                y₀
            )
            ...
            for j in all_coords[2:(end-1)]
                int_idx += 1
                p[:node_pos][][j.nid] = GeometryBasics.Point{2, Float32}(#
                    xl₀ + (int_idx * interval),
                    y₀
                )
            end
...
function buchheim_layout(
    ...
    A, E, t = trie_adj_at_root(T, false, Int16(I))
    ...
    G = SimpleDiGraph(A)
    f2, ax2, p2 = graphplot(G, layout = Buchheim()); # do not plot!
