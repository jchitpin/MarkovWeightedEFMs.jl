...
Plot cycle-history Markov chain from transition probability matrix `T` rooted
on state/node `I`.
...
function plot_chmc(#
    ...
    A, E, t = trie_adj_at_root(T, show_all, Int16(I))

    # Vectors of node and edge colours
    ...
    x1, x2, y1, y2 = buchheim_layout(p, T, 1)
    ...
    # Remove plot decorations and center axes on CHMC
    hidedecorations!(ax)
    hidespines!(ax)
